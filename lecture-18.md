...
CSS `transform` is a good property to animate (e.g. for [mobile menus](https://github.com/front-end-materials/menus)) because the GPU (Graphics Processing Unit) in both mobile and desk/laptops can be used.

The following avoid screen redraw and are the **least processor-intensive to animate**:
...
- **Specifications**: document how features should work in browsers, but also have their own ideas
...
.eye:hover {
  animation: catseye 4s ease-in-out infinite alternate;
}
@keyframes catseye {
...
<svg version="1" id="cat" viewBox="0 0 720 800" aria-labelledby="catTitle catDesc" role="img">
  <title id="catTitle">Pixels, the SVG cat</title>
...
- [SVG can do that?! (talk, Sarah Drasner)](https://www.youtube.com/watch?v=4laPOtTRteI) ([Slides](https://slides.com/sdrasner/svg-can-do-that#/))
- [Building Better Interfaces with SVG (presentation, Sarah Soueidan)](http://slides.com/sarasoueidan/building-better-interfaces-with-svg#/)
